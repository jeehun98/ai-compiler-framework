...
# ------------------------------------------------------------
def main():
    print("=== PR3 verify (model.compile API): compile+lower+capture+trace + determinism(full restore) ===")
    ...
    # 1) model.compile (auto train_step: mse)
    art = model.compile(
        optim=optim,
        x=x,
        t=t,
    ...
    # 5) Optional: show usage of model.replay()
    # for _ in range(3):
    #     model.replay()
    # torch.cuda.synchronize()
