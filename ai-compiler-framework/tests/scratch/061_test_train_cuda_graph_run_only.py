...
# ----------------------------
# 3) Run loops (NO compile-once; rely on executor's compile cache)
# ----------------------------
...
def run_steps_run_only(exe, feed, *, use_cuda_graph: bool, steps: int):
    ws = []
    steps_out = []

    for i in range(steps):
        feed_i = dict(feed)
        feed_i["g"] = torch.randn_like(feed_i["g"])  # new gradient each iter
        ...
        out = exe.run(
            ...
            mode="train",
            warmup=0,  # keep step progression obvious
        )
...
ws_e, st_e = run_steps_run_only(exe_eager, feed0_eager, use_cuda_graph=False, steps=3)
ws_g, st_g = run_steps_run_only(exe_graph, feed0_graph, use_cuda_graph=True, steps=3)
...
# ----------------------------
# 4) ATTACK TEST (run-only):
#    param/state should NOT be overwritten from feed on replay in train mode.
# ----------------------------
feed_attack = make_feed(999)

out1 = exe_graph.run(m, feed_attack, use_cuda_graph=True, mode="train", warmup=0)
torch.cuda.synchronize()
...
out2 = exe_graph.run(m, feed_attack2, use_cuda_graph=True, mode="train", warmup=0)
torch.cuda.synchronize()
s_after = int(out2["step"].item())

step_ok = (s_after > s_before)
not_reset = not torch.allclose(out2["w"], torch.zeros_like(out2["w"]), atol=1e-2, rtol=1e-2)
