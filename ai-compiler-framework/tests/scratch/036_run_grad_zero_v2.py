...
def run(dtype: str, shape):
    ...
    y = m.add(aicf.GradZero(name="gz", inplace=False), x)
    m.output("y", y)
    ...
    d = maxabs_delta(Y.float() if torch_dtype == torch.float16 else Y,
                     ref.float() if torch_dtype == torch.float16 else ref)

    print(f"[{dtype}] oop shape={tuple(shape)} max|delta|={d:.3e}")
    return d
...
def main():
    ...
    print("[NEG] keep dtype/shape + inplace alias tests in _C-only probe for now.")
