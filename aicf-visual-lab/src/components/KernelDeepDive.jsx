...
import { X, TrendingUp, Cpu, Activity, CheckCircle2 } from "lucide-react";
...
const KernelDeepDive = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div
      ...
      onMouseDown={onClose} // ✅ 오버레이 클릭 시 닫기
    >
      <div
        className="bg-[#1e293b] w-full max-w-4xl max-h-[90vh] rounded-[2rem] border border-slate-700 shadow-2xl overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between p-8 border-b border-slate-700 bg-[#0f172a]">
          <div>
            <div className="flex items-center gap-3 text-emerald-400 mb-2">
              <Activity size={24} />
              <h2 className="text-sm font-black tracking-[0.3em]">
                최적화 연대기
              </h2>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent">
          {/* 1️⃣ 버전 히스토리 */}
          <section>
            <h3 className="text-slate-400 font-bold tracking-widest text-xs mb-6 flex items-center gap-2">
              <TrendingUp size={16} /> 버전 히스토리
            </h3>

            {history.length > 0 ? (
              <div className="space-y-6 relative pl-4 border-l-2 border-slate-700 ml-2">
                {history.map((ver, idx) => (
                  <div key={`${ver.version ?? idx}-${idx}`} className="relative pl-8 group">
                    <div
                    ...
                    <div className="bg-[#0f172a] p-6 rounded-2xl border border-slate-800 group-hover:border-emerald-500/30 transition-all">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <span
                            className={`text-[10px] font-black px-2 py-1 rounded-md mr-3 ${
                              idx === history.length - 1
          ...
          <section>
            <h3 className="text-slate-400 font-bold tracking-widest text-xs mb-6 flex items-center gap-2">
              <Cpu size={16} /> 프로파일링 지표 (최신)
            </h3>

            {Object.keys(profiling).length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {Object.entries(profiling).map(([key, value]) => (
                  <div
                    key={key}
                    className="bg-[#0f172a]/50 p-4 rounded-xl border border-slate-800 text-center"
                  >
                    <p
                      className="text-[9px] text-slate-500 font-black tracking-tight mb-2 truncate"
                      title={key}
                    >
                      {key.replace(/_/g, " ")}
                    </p>
          ...
          <section className="bg-emerald-900/10 p-6 rounded-2xl border border-emerald-500/20">
            <h3 className="text-emerald-400 font-bold tracking-widest text-xs mb-3 flex items-center gap-2">
              <CheckCircle2 size={16} /> 분석 및 결론
            </h3>

            <p className="text-emerald-100/80 text-sm leading-relaxed font-medium">
              {analysis || "분석 내용이 아직 없습니다."}
            </p>
          </section>
        </div>
      </div>
