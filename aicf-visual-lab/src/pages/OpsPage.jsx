...
import {
  ...
  Terminal, Scale, Eye, Focus, History, Boxes, Menu, ArrowUpRight
} from 'lucide-react';
    ...
    <div className="flex min-h-dvh bg-[#0f172a] text-slate-200 antialiased overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-8 lg:p-10 space-y-12 pb-32 bg-gradient-to-b from-[#0f172a] to-[#1e293b]/20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 flex flex-col gap-6 min-w-0">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <MetricCard title="Throughput" value={km.throughput} color="text-emerald-400" icon={<Activity size={16} />} />
                  <MetricCard title="Mem Reuse" value={km.memory_reuse} color="text-purple-400" icon={<Layers size={16} />} />
                </div>
                ...
                <div className="bg-[#1e293b] p-6 sm:p-8 rounded-[2.5rem] border border-slate-800 shadow-xl">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {Object.entries(data.costModel?.weights_hint?.default ?? {}).map(([k, v]) => (
                      <div key={k} className="flex flex-col items-center gap-2 p-4 bg-[#0f172a]/50 rounded-2xl border border-slate-800">
                        <div className="text-lg font-black text-slate-100">{v}</div>
                        <p className="text-[9px] text-slate-600 uppercase font-black tracking-tighter text-center break-words">{k}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {/* ✅ 빠진 부분: 이 페이지에서 OP 선택 UI가 sidebar에만 있으니, deep link 버튼 하나 추가하면 좋음 */}
          <section className="pt-2">
            <Link
              to={`/theory?op=${data.id}`}
              className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-blue-600/10 border border-blue-500/20 text-blue-300 font-black text-xs uppercase tracking-widest hover:bg-blue-600/20 transition"
            >
