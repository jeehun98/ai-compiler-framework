...
export const residualAddData = {
  ...
  descriptions: {
    ...
    hardware:
      "이 연산은 주로 same-shape pointwise merge family로 연결되며, 실제 in-place accumulate, fused add+norm, epilogue path merge 같은 구현 세부는 Deep Dive 계층에서 다룹니다.",
  },
  ...
  lowering: {
    chosen: {
      ...
      reason: [
        ...
        "\\text{따라서 } \\texttt{Fused\\_Epilogue\\_ResidualAdd} \\text{ family가 적합하다}",
      ],
      applied_rewrites: [
  ...
  kernel: {
    strategy: "Pointwise Path Merge",
    details: [
      {
        technique: "Epilogue Residual Merge",
        semantic_link: "선행 연산 결과와 identity path를 writeback 직전에 병합",
      },
      {
        technique: "Vectorized Same-Shape Add",
        semantic_link: "동일 shape 경로 병합을 효율적으로 수행",
      },
      {
        technique: "Optional In-Place Accumulate",
        semantic_link: "합법한 경우 identity buffer를 결과 버퍼로 재사용",
      },
      {
        technique: "Fused Add+Norm Path",
        semantic_link: "병합 직후 normalization이 이어질 때 block-level fusion 가능",
      },
    ],
    metrics: {
      memory_reuse: "High When Fused",
      throughput: "Memory Bound / Near-Free When Piggybacked",
      occupancy: "High",
    },
  },
